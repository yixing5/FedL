...
LastEditors: Sasha
LastEditTime: 2023-03-13 15:35:16
Description: 
FilePath: /FederatedLearning/FedL/federatedml/linear_model/linear_regression/linr_host.py
'''

import numpy as np
from federatedml.data_factory import split_data
...
class LinrHost(ModelBase):
    ...
    def cumputer_ua_L(self):
        ...
        return encrypted_u_a,self.public_key.encrypt(L_a)
        # 计算加密梯度
    ...
    def fit(self,n):
        ...
        # 获取公钥
        #############################################计算[[u_a]],[[L_a]]发送给B方#############################################################
        encrypted_u_a,encrypted_L_a = self.cumputer_ua_L()
        self.remote(encrypted_L_a,"encrypted_L_a_%s" % self.n,"50052") 
