...
import numpy as np
import gmpy2
RANDOM_SIZE = 1024*10*10
random_pool = Queue(RANDOM_SIZE)

class RandomNumManager():
    ...
    @staticmethod
    def qsize():
        return random_pool.qsize()
    
    def consumer(self,size):
        return [random_pool.get() for _ in range(size)]

    def producer(self,pool,hs,max_alpha,nsquare):
        while True:
            value = gmpy_math.powmod(hs, random.randint(2, max_alpha - 1), nsquare)
            pool.put(value)
                
    def handle(self,hs,max_alpha,nsquare):
        producers = []
        for _ in range(self.producer_num):
            p = Process(target=self.producer,args=(random_pool,hs,max_alpha,nsquare))
            p.daemon=True
            producers.append(p)
        for p in producers:
            p.start()
...
class PaillierKeypair:
    ...
    def _generate_p_q(key_size):
        ...
        while n_len != key_size:
            p = gmpy_math.getprimeover(key_size // 2)
            while gmpy2.mod(p, 4) != 3:
                p = gmpy_math.getprimeover(key_size // 2)
            q = p
            while q == p:
                q = gmpy_math.getprimeover(key_size // 2)
    ...
    def generate_keypair(n_length):
        p, q = PaillierKeypair._generate_p_q(n_length)
        n = p * q
        x = random.randint(2, min(p, q))
        h = -pow(x, 2)
        n_square = pow(n, 2)
        hs = gmpy2.powmod(h, n, n_square)
        max_alpha = pow(2, int(gmpy2.ceil(n_length // 2)))
        public_key = PaillierPublicKey(n=n, hs=hs, max_alpha=max_alpha)
        private_key = PaillierPrivateKey(public_key=public_key, p=p, q=q)
...
class PaillierPublicKey(object):
    ...
    """
    def __init__(self, n,hs,max_alpha):
        self.g = n + 1
        self.n = n
        self.hs = hs
        ...
        self.nsquare = n * n
        self.max_int = n // 3 - 1
        self.rnm = RandomNumManager(RANDOM_SIZE)
        self.rnm.handle(self.hs,self.max_alpha,self.nsquare)
    def __repr__(self):
    ...
    def consumer(self,size=1):
        if random_pool.qsize() < size:
            return gmpy_math.powmod(self.hs, random.randint(2, self.max_alpha - 1), self.nsquare)
        return self.rnm.consumer(size)
    
    def apply_obfuscator(self, ciphertext, random_value=None):
        """
        """
        obfuscator = random_value or gmpy_math.powmod(self.hs, random.randint(2, self.max_alpha - 1), self.nsquare)
        # obfuscator = random_value or self.consumer()
        return int(gmpy2.mod(ciphertext*obfuscator, self.nsquare))
    ...
    def encrypt(self, value, precision=None, random_value=None):
        """Encode and Paillier encrypt a real number value. obfuscator
        """
    ...
    def encrypt_list(self,arr):
        ...
        arr = arr.flatten()
        result = []
        if random_pool.qsize() < arr.shape[0]:
            result = multi_task.map(self.encrypt,[(i,None,None)for i in arr])
        else:
            result = [self.encrypt(i,None,self.consumer()[0]) for i in arr]
...
class PaillierEncryptedNumber(object):
    ...
    def __raw_add(self, e_x, e_y, exponent):
        ...
        ciphertext = int(gmpy2.mod(e_x * e_y, self.public_key.nsquare))
        return PaillierEncryptedNumber(self.public_key, ciphertext, exponent)
